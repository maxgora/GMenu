...
static void MenuUpdate(ButtonPushType btn)
{
  ...
  const uint8_t dy = 25;          // приращение y-координаты
...
ScreenReturnType Screen0Pool(ButtonPushType btn)
{
  ...
  DrawIcoDownFill(ico_down.x, ico_down.y, bg_color);
  DrawIcoUpFill(ico_up.x, ico_up.y, bg_color);
  ...
  switch (btn)
  {
    case BTN_DOWN:
      ...
      break;
    case BTN_PUSH_DOWN:
      DrawIcoDownFill(ico_down.x, ico_down.y, Black);
      break;
    ...
    case BTN_UP:
      ...
      break;
    case BTN_PUSH_UP:
      DrawIcoUpFill(ico_up.x, ico_up.y, Black);
      break;
    ...
    case BTN_SET:
      switch (menu_iactive)
      {
        ...
        case 4: ret = SCREENx_RET_TO_SCREEN6; break;
...
void Screen0Init(gColor bc, gColor c)
{  
  bg_color = bc;
  color = c;
