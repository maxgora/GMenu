...
#define ST7735_COL_SHIFT 1
#define ST7735_ROW_SHIFT 2
...
#define write_data_repeat(g, data, count) { int i; for (i = 0; i < count; ++i) write_data_word (g, data); }
#endif
...
static const unsigned char
  init_cmds[] = {                 // Init for 7735R, part 1 (red or green tab)
    ...
    ST7735_MADCTL , 1      ,  // 14: Memory access control (directions), 1 arg:
      0xa0|ST7735_MADCTRL_COLOR, //     row addr/col addr, bottom to top refresh
...
static void set_viewport(GDisplay *g) {
	...
	write_data_word (g, g->p.x+ST7735_COL_SHIFT);
	write_data_word (g, g->p.x+g->p.cx-1+ST7735_COL_SHIFT);
	write_cmd (g, ST7735_RASET);
	write_data_word (g, g->p.y+ST7735_ROW_SHIFT);
	write_data_word (g, g->p.y+g->p.cy-1+ST7735_ROW_SHIFT);
